...
function displayEditUsers($db_con, $user_id, $msg = NULL)
{
	...
	echo "<input type='password' id='password' name='password' value = \"". htmlentities ( $row ["password"] ) ."\" required /></p>\n";
...
function updateUserDetails($db_con, $user_id, $fname, $lname, $username, $email, $password, $className)
{
	...
	if(!($fname && $lname && $username && $email && $password && $className)){
		header("Location:editusers.php?editUser=$user_id&Msg=You need to fill in all fields - Please try again");
		exit();
	}
	//If all variables have been entered, update the user details and report a success message back to the user
	else {
