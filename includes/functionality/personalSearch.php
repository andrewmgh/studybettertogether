...
if ($_SERVER['REQUEST_METHOD']== "POST") //only runs the below php code if the login form was submitted. 
{
	if (isset ($_POST ['search'])) {
		...
		if($fileName || $fileOwner || $fileType || $description || $subject){
				...
				$searchResults = personalSearch($db_con, $username, $fileName, $fileOwner, $fileType, $description, $subject);
...
function personalSearch($db_con, $username, $fileName, $fileOwner, $fileType, $description, $subject) {
...
//($row ["file_short_name"] !== "PDF File") ? $searchResults .= "<td>" . ProtectURL(( $row ["file_path"] )) ."</td></tr>\n" : $searchResults .= "<td></td></tr>\n";
