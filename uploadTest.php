...
if ($_SERVER['REQUEST_METHOD']== "POST") //only runs the below php code if the login form was submitted.
{
	//$specificData = "";
	$uploadMsg = "FIRST";
	$sharingStatus = $_POST['sharingStatus'];
	if(isset($_POST ['specificSharing'])){$specificData = $_POST['specificSharing'];}
	
	//if(isset($sharingStatus)){
	
		$uploadMsg= validateSpecificSharing($sharingStatus, $specificData);
		echo  $uploadMsg;
...
function validateSpecificSharing ($sharingStatus, $specificData){
	if(($sharingStatus == "specific") && ($specificData == "") ){
